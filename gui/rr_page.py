...
class RRPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(
            header, text="🔹 Round Robin (RR) Scheduling",
            font=("Orbitron", 26, "bold"), text_color="#06b6d4"
        ).pack(side="left", padx=30, pady=10)

        ctk.CTkButton(
            header, text="⬅ Back to Menu",
            fg_color="#1f2833", hover_color="#06b6d4",
        ...
        self.quantum = ctk.IntVar(value=2)

        ctk.CTkLabel(
            self.input_frame, text="Number of Processes:",
            text_color="#06b6d4", font=("Helvetica", 14, "bold")
        ...
        ctk.CTkLabel(
            self.input_frame, text="Time Quantum:",
            text_color="#06b6d4", font=("Helvetica", 14, "bold")
        ).grid(row=0, column=2, padx=10, pady=10, sticky="e")

        ctk.CTkEntry(self.input_frame, textvariable=self.quantum, width=100).grid(
            row=0, column=3, padx=10, pady=10, sticky="w"
        )

        ctk.CTkButton(
            self.input_frame, text="Set Processes",
            fg_color="#06b6d4", hover_color="#67e8f9",
            text_color="#0b0c10", corner_radius=12,
    ...
    def make_process_table(self):
        ...
        for i, h in enumerate(headers):
            ctk.CTkLabel(self.table_frame, text=h, text_color="#06b6d4",
                         font=("Helvetica", 13, "bold")).grid(row=0, column=i, padx=10, pady=5)
        for i in range(n):
    ...
    def run_simulation(self):
        try:
            ...
            q = self.quantum.get()
            result, gantt = run_simulation("RR", processes, q)
            ...
            ctk.CTkLabel(
                ...
                font=("Helvetica", 15, "bold"), text_color="#06b6d4"
            ).pack(pady=20)
    ...
    def reset_page(self):
        ...
        self.quantum.set(2)
        for f in [self.table_frame, self.result_frame]:
